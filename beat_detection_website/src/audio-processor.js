...
class BeatDetector extends AudioWorkletProcessor {
    ...
    getEnergy(audio_data_fft) {
        ...
        for (let i = 0; i < TOTAL_SUB_BANDS; i++) {
            instant_energy.push(Math.pow(Math.abs(audio_data_fft[i]), 3));
        }
    ...
    // ===========================================================================
    // Method:  Checks if a beat has occurred and makes that position in the array true
    ...
    checkBeat(instant_energy, energy_history, sub_band_beat) {
        ...
        const energy_history_f = new Array(HISTORY_CHUNKS)
            .fill(0)
            .map(() => new Array(TOTAL_SUB_BANDS).fill(0));

        // Find the max
        let max_instant = 0;
        for (let i = 0; i < instant_energy.length; i++) {
            if (instant_energy[i] > max_instant) {
                max_instant = instant_energy[i];
            }
        }
        ...
        // Normalize the energy history
        for (let i = 0; i < HISTORY_CHUNKS; i++) {
            for (let j = 0; j < TOTAL_SUB_BANDS; j++) {
                energy_history_f[i][j] = energy_history[i][j] / max;
            }
        }
        ...
        for (let i = 0; i < TOTAL_SUB_BANDS; i++) {
            for (let j = 0; j < HISTORY_CHUNKS; j++) {
                average_energies[i] += energy_history_f[j][i];
            }
        ...
        for (let i = 0; i < TOTAL_SUB_BANDS; i++) {
            if (
                instant_energy_f[i] > (thresholds[i] * average_energies[i]) / 1.15 ||
    ...
    process(inputs, outputs) {
        this.port.onmessage = ({ data }) => {
            ...
            if (this._sub_band_beat[0]) {
                if (this._chunks_processed - this._bass_chunk > 8) {
                    if (this._beat_history[0].length >= 4) {
            ...
            if (
                ...
                if (this._chunks_processed - this._clap_chunk > 6) {
